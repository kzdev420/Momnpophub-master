...
class ContactsList extends Component {
  ...
  getCustomerList() {
    ...
    firebaseService.onAuthStateChanged(authUser => {
      if (authUser) {
        firebaseService.adminGetAllCustomers().then(customerdata => {
          ...
          <div className="flex items-center">
            {row.original.status ? (
              <IconButton
                onClick={ev => {
                  ev.stopPropagation();
                  this.changeStatusCustomer(row.original.id, false);
                }}
              >
                <Icon>toggle_on</Icon>
              </IconButton>
              ...
              <IconButton
                onClick={ev => {
                  ev.stopPropagation();
                  this.changeStatusCustomer(row.original.id, true);
                }}
              >
            ...
            {row.original.disable ? (
              <IconButton
                onClick={ev => {
                  ev.stopPropagation();
                  this.disEnabCustomer(row.original.id, false);
                }}
              >
              ...
              <IconButton
                onClick={ev => {
                  ev.stopPropagation();
                  this.disEnabCustomer(row.original.id, true);
                }}
              >
                <Icon>lock_open</Icon>
              </IconButton>
            ...
            <IconButton
              onClick={ev => {
                ev.stopPropagation();
                this.editCustomer(row.original.id);
              }}
            >
              <Icon>edit</Icon>
            </IconButton>
            <IconButton
              onClick={ev => {
                ev.stopPropagation();
                this.deleteCustomer(row.original.id);
              }}
            >
              <Icon>delete</Icon>
            </IconButton>
        ...
        if (this.state.customerListStatus) {
          ...
          if (!customerdata.status) {
            customerArray.push({
              ...
              coupons: customerdata.couponCount || 0,
              createdAt: customerdata.createdAt
                ? new Date(customerdata.createdAt.toDate()).toLocaleString("en-US")
                : "N/A"
            });
          }
        }
...
class ContactEdit extends Component {
  ...
  componentWillMount() {
    ...
    firebaseService.onAuthStateChanged(authUser => {
      if (authUser) {
        firebaseService.getspecificCustomer(this.props.customerID).then(customerdata => {
